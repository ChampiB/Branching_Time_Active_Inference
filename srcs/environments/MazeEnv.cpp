...
namespace btai::environments {
    ...
    std::shared_ptr<MazeEnv> MazeEnv::create(const std::string &file) {
    ...
    torch::Tensor MazeEnv::execute(int action) {
        ...
        return Torch::oneHot(observations(), manhattan_distance(agent_pos));
    }
    ...
    void MazeEnv::loadStatesIndexes() {
        ...
        states_idx = Torch::full(maze.sizes(), -1).to(kInt);
