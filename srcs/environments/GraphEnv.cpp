...
namespace btai::environments {
    ...
    std::shared_ptr<GraphEnv> GraphEnv::create(int nGood, int nBad, const std::vector<int> &sizeGoodPaths) {
    ...
    Tensor GraphEnv::reset() {
        agent_state = 0;
        return Torch::oneHot(observations(), ObsType::GOOD);
    }

    Tensor GraphEnv::execute(int action) {
        if (action < 0 or action >= actions())
            throw std::runtime_error("GraphEnv: invalid action.");

        agent_state = execute(action, agent_state);
        return Torch::oneHot(observations(), (agent_state == 1) ? ObsType::BAD : ObsType::GOOD);
    }
    ...
    void GraphEnv::print() const {
        ...
        // Create the strings containing the paths' name, i.e. Path_N where N is the path index.
    ...
    torch::Tensor GraphEnv::B() {
        Tensor B = Torch::full({states(), states(), actions()}, 0.1 / (states() - 1));
    ...
    int GraphEnv::execute(int action, int state) const {
        ...
        throw std::runtime_error("Invalid action or state, when calling GraphEnv::execute(" + std::to_string(action) + ", " + std::to_string(state) + ")");
    }

    bool GraphEnv::solved() {
