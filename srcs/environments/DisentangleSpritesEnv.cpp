...
namespace btai::environments {
    ...
    DisentangleSpritesEnv::DisentangleSpritesEnv(const std::string &path, int gran, int rep) {
        ...
        s_bases_obs = Torch::tensor({0, n_pos * (n_pos + 1), 0, 0, n_pos + 1, 1});
        s_bases_img = Torch::tensor({737280, 245760, 40960, 1024, 32, 1});
        state = Torch::zeros({s_dim});
    }
    ...
    torch::Tensor DisentangleSpritesEnv::current_observation() const {
        return Torch::oneHot(observations(), get_obs_id(state));
    }
    ...
    Tensor DisentangleSpritesEnv::A() {
        double noise = 0.01;
        double epsilon = noise / (observations() - 1);
        Tensor res = Torch::full({observations(), observations()}, epsilon);
    ...
    Tensor DisentangleSpritesEnv::B() {
        int n_states = states();
        int n_actions = actions();
        Tensor B = Torch::full({n_states, n_states, n_actions}, 0.1 / (n_states - 1));
    ...
    Tensor DisentangleSpritesEnv::D() {
    ...
    void DisentangleSpritesEnv::print() const {
    ...
    torch::Tensor DisentangleSpritesEnv::C(double prefPrecision) const {
        Tensor C = Torch::zeros({observations()});
        Tensor bottom_state = Torch::tensor({0, 0, 0, 0, 0, 32});
    ...
    bool DisentangleSpritesEnv::solved() {
        return need_reset;
    }
    ...
    Tensor DisentangleSpritesEnv::getStateFromId(int i) const {
        torch::Tensor cur_state = Torch::tensor({0, 0, 0, 0, 0, 0});
